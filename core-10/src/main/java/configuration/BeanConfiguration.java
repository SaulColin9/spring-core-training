...
public class BeanConfiguration {
    @Bean
    static CarSpecGenerator carSpecGenerator() {
        return new CarSpecGenerator();
    }
    ...
    @Profile("sport")
    ChevroletCamaro chevroletCamaro(@Qualifier("powerOverwhelmingStuff") Engine engine,
                                    @Value("#{ T(java.lang.Math).random()*101.0 + 200.0 }") int maxSpeed) {
    ...
    VolkswagenBeetle Bumblebee(@Qualifier("oldSchoolStuff") Engine engine,
                               @Value("#{ '${volkswagen.oil}' + ' Premium'}") String oilName) {
